...
ylabel('Average AoI');
lgd = legend('Ber-FCFS-ana(\lambda=1/3)', 'Ber-pLGFS-ana(\lambda=1/3)', ...
    'per-FCFS-ana(K=3)', 'per-pLGFS-ana(K=3)', ...
    'Ber-FCFS-simu(\lambda=1/3)', 'Ber-pLGFS-simu(\lambda=1/3)', ...
    'per-FCFS-simu(K=3)', 'per-pLGFS-simu(K=3)');
lgd.NumColumns = 2;
save memoryAoI.mat
